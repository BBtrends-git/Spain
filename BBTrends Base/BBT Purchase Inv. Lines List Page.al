Page 50010 "Purch Inv. Lines List"
{
    // //INC-2018-06-93710 - Page nueva líneas de histórico de facturas compra
    Caption = 'Purch Inv. Lines';
    DeleteAllowed = false;
    InsertAllowed = false;
    ModifyAllowed = false;
    PageType = List;
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(NumfacturaProveedor; NumfacturaProveedor)
                {
                    ApplicationArea = Basic;
    ...
    begin
        Clear(Divisa);
        Clear(NumfacturaProveedor);
        TcabCompraFactura.Reset;
        if TcabCompraFactura.Get(Rec."Document No.")then begin
            NumfacturaProveedor:=TcabCompraFactura."Vendor Invoice No.";
            Divisa:=TcabCompraFactura."Currency Code";
        end;
    end;
    var NumfacturaProveedor: Code[35];
    TcabCompraFactura: Record "Purch. Inv. Header";
    Divisa: Code[10];
}
