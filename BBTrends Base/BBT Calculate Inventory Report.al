Report 50010 "BBT Calculate Inventory"
//Se clona este report porque se modifica tanto impresión como código del estándar
//fin - The application object identifier '204' is not valid. It must be within the allowed ranges '[50000..90500]'.
//    - An application object of type 'Report' with name 'Calculate Inventory is already declared by the extension 'Base Application by Microsoft (18.0.23013.23795)'
{
    Caption = 'Calculate Inventory', comment = 'ESP="Calcular inventario físico"';
    ProcessingOnly = true;
        ...
        dataitem(Item; Item)
        {
            ...
            dataitem("Item Ledger Entry"; "Item Ledger Entry")
            {
                ...
                begin
                    ...
                    if ("Location Code" <> '') and Location."Use As In-Transit" then CurrReport.Skip();
                        ...
                        if ByBin then begin
                            if not TempSKU.Get("Location Code", "Item No.", "Variant Code") then begin
                                ...
                                if "Variant Code" = '' then
                                    InsertTempSKU := true
                                else if ItemVariant.Get("Item No.", "Variant Code") then InsertTempSKU := true;
                ...
                begin
                    ...
                    if IncludeItemWithNoTransaction then
                        if not "Item Ledger Entry".Find('-') then begin
                            WhseEntry.SetRange("Item No.", Item."No.");
                            if (Item.GetFilter("Variant Filter") = '') and (Item.GetFilter("Location Filter") = '') and WhseEntry.IsEmpty then begin
            ...
            dataitem("Warehouse Entry"; "Warehouse Entry")
            {
                ...
                begin
                    ...
                    if Location."Bin Mandatory" and not Location."Directed Put-away and Pick" then TempQuantityOnHandBuffer."Bin Code" := "Bin Code";
                    OnBeforeQuantityOnHandBufferFindAndInsert(TempQuantityOnHandBuffer, "Warehouse Entry");
                    if not TempQuantityOnHandBuffer.Find() then TempQuantityOnHandBuffer.Insert(); // Insert a zero quantity line.
                end;
            }
            ...
            begin
                ...
                IF _InfoCompany.SGA THEN BEGIN
                    FiltroAlmacen := Item.GETFILTER("Location Filter");
                    InventorySetup.GET;
                    InventorySetup."Almacén último inventario" := FiltroAlmacen;
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(PostingDate; PostingDate)
                    {
                        ...
                        ToolTip = 'Specifies the date for the posting of this batch job. By default, the working date is entered, but you can change it.', comment = 'ESP="Especifica la fecha de registro de este proceso. La fecha de trabajo se especifica de forma predeterminada, pero es posible cambiarla."';
                    ...
                    field(DocumentNo; NextDocNo)
                    {
                        ...
                        ToolTip = 'Specifies the number of the document that is processed by the report or batch job.', comment = 'ESP="Especifica el número del documento procesado por el informe o el trabajo por lotes."';
                    }
                    field(ItemsNotOnInventory; ZeroQty)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Items Not on Inventory.', comment = 'ESP="Incluir prods. sin invent."';
                        ToolTip = 'Specifies if journal lines should be created for items that are not on inventory, that is, items where the value in the Qty. (Calculated) field is 0.', comment = 'ESP="Especifica si deben crearse líneas del diario para los artículos que no están en el inventario, es decir, artículos en los que el valor del campo Stock calculado es 0."';
                    ...
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Include Item without Transactions', comment = 'ESP="Incluir artículo sin transacciones"';
                        ToolTip = 'Specifies if journal lines should be created for items that are not on inventory and are not used in any transactions.', comment = 'ESP="=Especifica si deben crearse líneas del diario para los artículos que no están en el inventario y que no se usan en ninguna transacción."';
                    ...
                    field(ByDimensions; ColumnDim)
                    {
                        ...
                        Caption = 'By Dimensions', comment = 'ESP="Por dimensiones"';
                        Editable = false;
                        ToolTip = 'Specifies the dimensions that you want the batch job to consider.', comment = 'ESP="Especifica las dimensiones que desea que tenga en cuenta el proceso."';
    ...
    var
        ...
        PosQty: Decimal;
        NegQty: Decimal;
        ItemNotOnInventoryErr: Label 'Items Not on Inventory.', comment = 'ESP="Incluir prods. sin invent."';
        Text000: Label 'Enter the posting date.', comment = 'ESP="Introduzca la fecha de registro."';
        Text001: Label 'Enter the document no.', comment = 'ESP="IIntroduzca el número de documento."';
        Text002: Label 'Processing items    #1##########', comment = 'ESP="Procesando prods.   #1##########"';
        Text003: Label 'Retain Dimensions', comment = 'ESP="Retener dimensiones"';
        FiltroAlmacen: Text;
    ...
    begin
        if not ItemJnlBatch.Get(ItemJnlLine."Journal Template Name", ItemJnlLine."Journal Batch Name") then exit;
    ...
    begin
        ...
        if ShouldInsertItemJnlLine then begin
            ...
            //>> SDA 20230328. Filtro directo para generar inventario.
            //    "Phys Invt Counting Period Code" := PhysInvtCountCode;
            IF PhysInvtCountCode <> '' THEN
                ItemJnlLine."Phys Invt Counting Period Code" := PhysInvtCountCode
            ELSE BEGIN
                rItem.INIT;
                rItem.SETRANGE("No.", ItemNo);
                IF rItem.FINDFIRST THEN ItemJnlLine."Phys Invt Counting Period Code" := rItem."Phys Invt Counting Period Code";
            END;
            //<<
            ItemJnlLine."Phys Invt Counting Period Code" := PhysInvtCountCode;
            ...
            if Location.Code <> '' then if Location."Directed Put-away and Pick" then ReserveWarehouse(ItemJnlLine);
        ...
        if WhseEntry.Find('-') then
            repeat
                ...
                if WhseEntry."Qty. (Base)" <> 0 then begin
                    ...
                    CreateReservEntry.CreateReservEntryFor(DATABASE::"Item Journal Line", ItemJnlLine."Entry Type".AsInteger(), ItemJnlLine."Journal Template Name", ItemJnlLine."Journal Batch Name", OrderLineNo, ItemJnlLine."Line No.", ItemJnlLine."Qty. per Unit of Measure", Abs(WhseEntry.Quantity), Abs(WhseEntry."Qty. (Base)"), ReservEntry);
                    ...
                    CreateReservEntry.CreateEntry(ItemJnlLine."Item No.", ItemJnlLine."Variant Code", ItemJnlLine."Location Code", ItemJnlLine.Description, 0D, 0D, 0, "Reservation Status"::Prospect);
                end;
                WhseEntry.Find('+');
    ...
    begin
        ...
        if not ShouldAddZeroQty then exit;
        ...
        if TempQuantityOnHandBuffer.FindSet() then begin
            repeat
                ...
                if (NegQty - TempQuantityOnHandBuffer.Quantity <> TempQuantityOnHandBuffer.Quantity - PosQty) or ItemTrackingSplit then begin
                    if PosQty = TempQuantityOnHandBuffer.Quantity then PosQty := 0;
                    if (PosQty <> 0) or AdjustPosQty then InsertItemJnlLine(TempQuantityOnHandBuffer."Item No.", TempQuantityOnHandBuffer."Variant Code", TempQuantityOnHandBuffer."Dimension Entry No.", TempQuantityOnHandBuffer."Bin Code", TempQuantityOnHandBuffer.Quantity, PosQty);
                    if NegQty = TempQuantityOnHandBuffer.Quantity then NegQty := 0;
                    if NegQty <> 0 then begin
                        ...
                        InsertItemJnlLine(TempQuantityOnHandBuffer."Item No.", TempQuantityOnHandBuffer."Variant Code", TempQuantityOnHandBuffer."Dimension Entry No.", TempQuantityOnHandBuffer."Bin Code", TempQuantityOnHandBuffer.Quantity, NegQty);
                        ZeroQty := ZeroQtySave;
                    end;
                end
                else begin
                ...
                if (PosQty = 0) and (NegQty = 0) and not AdjustPosQty then InsertItemJnlLine(TempQuantityOnHandBuffer."Item No.", TempQuantityOnHandBuffer."Variant Code", TempQuantityOnHandBuffer."Dimension Entry No.", TempQuantityOnHandBuffer."Bin Code", TempQuantityOnHandBuffer.Quantity, TempQuantityOnHandBuffer.Quantity);
