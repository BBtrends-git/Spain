report 50016 "BBT Calculate Inventory Value"
{
    Caption = 'BBT Calculate Inventory Value';
    ProcessingOnly = true;
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            var
                ...
                RemCost: Decimal;
            begin
                if ShowDialog then Window.Update();
                SkipItem := false;
                ...
                if (CalculatePer = CalculatePer::Item) and ("Costing Method" = "Costing Method"::Average) then begin
                    ...
                    if PostingDate <> CalendarPeriod."Period End" then Error(Text011, "No.", PostingDate, CalendarPeriod."Period End");
                end;
                        ...
                        if IncludeEntryInCalc(ItemLedgEntry, PostingDate, IncludeExpectedCost) then begin
                            ...
                            case CalculatePer of
                                CalculatePer::"Item Ledger Entry":
                                    InsertItemJnlLine(ItemLedgEntry."Entry Type", ItemLedgEntry."Item No.", ItemLedgEntry."Variant Code", ItemLedgEntry."Location Code", RemQty, RemCost, ItemLedgEntry."Entry No.", 0, ItemLedgEntry."Posting Date");
                                CalculatePer::Item:
                                    InsertValJnlBuffer(ItemLedgEntry."Item No.", ItemLedgEntry."Variant Code", ItemLedgEntry."Location Code", RemQty, RemCost);
                            end;
                        end;
                ...
                if CalculatePer = CalculatePer::Item then
                    ...
                    else if (GetFilter("Variant Filter") <> '') or ByVariant then begin
                        ByVariant2 := true;
                        ItemVariant.SetRange("Item No.", "No.");
                        CopyFilter("Variant Filter", ItemVariant.Code);
            ...
            begin
                if not UpdStdCost then exit;
                if ByLocation then CopyFilter("Location Filter", SKU."Location Code");
                if ByVariant then CopyFilter("Variant Filter", SKU."Variant Code");
                TempNewStdCostItem.CopyFilters(Item);
                if TempNewStdCostItem.Find('-') then
                    repeat
                        if not TempUpdatedStdCostSKU.Get('', TempNewStdCostItem."No.", '') then ItemCostMgt.UpdateStdCostShares(TempNewStdCostItem);
            ...
            begin
                ...
                if NextDocNo = '' then begin
                    if ItemJnlBatch."No. Series" <> '' then begin
                        ...
                        if not ItemJnlLine.FindFirst() then begin
                            ...
                            NextDocNo := NoSeries.GetNextNo(ItemJnlBatch."No. Series", PostingDate, false);
                            //<<
                        end;
                        ItemJnlLine.Init();
                    end;
                    if NextDocNo = '' then Error(Text003);
                end;
                //>> V27
                ...
                NextLineNo := 0;
                if ShowDialog then Window.Open(Text010, "No.");
                GLSetup.Get();
                SourceCodeSetup.Get();
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(CalculatePer; CalculatePer)
                    {
                        ...
                        trigger OnValidate()
                        begin
                            if CalculatePer = CalculatePer::Item then ItemCalculatePerOnValidate();
                            if CalculatePer = CalculatePer::"Item Ledger Entry" then ItemLedgerEntryCalculatePerOnV();
                        end;
                    }
    ...
    begin
        ...
        if IncludeExpectedCost then exit(ItemLedgEntry."Posting Date" in [0D .. PostingDate]);
    ...
    begin
        ...
        if TempValJnlBuffer.FindFirst() then begin
            ...
            TempValJnlBuffer."Inventory Value (Calculated)" := TempValJnlBuffer."Inventory Value (Calculated)" + Amount2;
            TempValJnlBuffer.Modify();
        end
        else if Quantity2 <> 0 then begin
    ...
    begin
        ...
        if TempValJnlBuffer.GetFilter("Location Code") <> '' then ValueEntry."Location Code" := TempValJnlBuffer.GetRangeMin("Location Code");
        if TempValJnlBuffer.GetFilter("Variant Code") <> '' then ValueEntry."Variant Code" := TempValJnlBuffer.GetRangeMin("Variant Code");
        ...
        if AverageQty <> 0 then begin
            ...
            if AverageUnitCostLCY < 0 then AverageUnitCostLCY := 0;
        end
        else
        ...
        if ItemLedgEntry.Find('-') then
            repeat
                if (ItemLedgEntry.Quantity = ItemLedgEntry."Invoiced Quantity") and
                    ...
                    (ItemLedgEntry."Invoiced Quantity" <> 0) then begin
    ...
    local procedure InsertItemJnlLine(EntryType2: Enum "Item Ledger Entry Type"; ItemNo2: Code[20]; VariantCode2: Code[10]; LocationCode2: Code[10]; Quantity2: Decimal; Amount2: Decimal; ApplyToEntry2: Integer; AppliedAmount: Decimal; PostingDateILE: Date)
    var
        Item: Record Item;
        SKU: Record "Stockkeeping Unit";
    ...
    begin
        ...
        case CalcBase of
            ...
            CalcBase::"Standard Cost - Assembly List", CalcBase::"Standard Cost - Manufacturing":
        ...
        ItemJnlLine.Insert();
        //end;
        Months := (Today - ItemJnlLine."Posting Date ILE") / 30;
        //Modificamos en base al porcetanej de depreciacion
        IF Depreciation.FindFirst() then begin
            repeat
                case Depreciation."Depreciation Range" of
                    Depreciation."Depreciation Range"::"0-6 months":
                        begin
                            IF Months <= 6 then begin
                                ItemJnlLine.VALIDATE("Unit Cost (Revalued)", ItemJnlLine."Unit Cost (Revalued)" - (ItemJnlLine."Unit Cost (Revalued)" * (Depreciation."Depreciation Percetange" / 100)));
                            end;
                        end;
                    Depreciation."Depreciation Range"::"12-18 months":
                        begin
                            IF (Months > 12) AND (Months <= 18) then begin
                                ItemJnlLine.VALIDATE("Unit Cost (Revalued)", ItemJnlLine."Unit Cost (Revalued)" - (ItemJnlLine."Unit Cost (Revalued)" * (Depreciation."Depreciation Percetange" / 100)));
                            end;
                        end;
                    Depreciation."Depreciation Range"::"18-24 months":
                        begin
                            IF (Months > 18) AND (Months <= 24) then begin
                                ItemJnlLine.VALIDATE("Unit Cost (Revalued)", ItemJnlLine."Unit Cost (Revalued)" - (ItemJnlLine."Unit Cost (Revalued)" * (Depreciation."Depreciation Percetange" / 100)));
                            end;
                        end;
                    Depreciation."Depreciation Range"::"6-12 months":
                        begin
                            IF (Months > 6) AND (Months <= 12) then begin
                                ItemJnlLine.VALIDATE("Unit Cost (Revalued)", ItemJnlLine."Unit Cost (Revalued)" - (ItemJnlLine."Unit Cost (Revalued)" * (Depreciation."Depreciation Percetange" / 100)));
                            end;
                        end;
                end;
            until Depreciation.Next() = 0;
            ItemJnlLine.Modify();
        end;
    end;
    ...
    local procedure InitItemJnlLine(var ItemJnlLine: Record "Item Journal Line"; EntryType2: Enum "Item Ledger Entry Type"; ItemNo2: Code[20]; VariantCode2: Code[10]; LocationCode2: Code[10]; PostingDateILE: Date)
    begin
        // Obsoleto With
        //with ItemJnlLine do begin
        ...
        ItemJnlLine."Posting Date ILE" := PostingDateILE;
        //end;
    end;
    ...
    begin
        ...
        case CalculatePer of
            CalculatePer::"Item Ledger Entry":
                exit(ItemLedgerEntry.CalculateRemInventoryValue(ItemLedgerEntry."Entry No.", ItemLedgerEntry.Quantity, RemainingQty, false, UntilValuationDate, UntilPostingDate));
            CalculatePer::Item:
                exit(ItemLedgerEntry.CalculateRemInventoryValue(ItemLedgerEntry."Entry No.", ItemLedgerEntry.Quantity, RemainingQty, IncludeExpCost and not ItemLedgerEntry."Completely Invoiced", UntilValuationDate, UntilPostingDate));
        end;
        exit(0);
    end;
    ...
    begin
        ...
        if TempValJnlBuffer.Quantity <> 0 then begin
            AppliedAmount := 0;
            if Item."Costing Method" = Item."Costing Method"::Average then CalcAverageUnitCost(TempValJnlBuffer.Quantity, TempValJnlBuffer."Inventory Value (Calculated)", AppliedAmount);
            InsertItemJnlLine(ItemJnlLine."Entry Type"::"Positive Adjmt.", Item."No.", VariantCode, LocationCode, TempValJnlBuffer.Quantity, TempValJnlBuffer."Inventory Value (Calculated)", 0, AppliedAmount);
        end;
    end;
}
