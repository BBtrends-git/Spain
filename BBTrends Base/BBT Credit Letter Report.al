report 50017 "BBT Credit Letter"
{
    ApplicationArea = All;
    Caption = 'Credit Letter', Comment = 'ESP="Carta de crédito"';
    UsageCategory = ReportsAndAnalysis;
    PreviewMode = PrintLayout;
    //WordMergeDataItem = Header;
    DefaultLayout = Word;
    WordLayout = './src/Report/Layouts/CreditLetter.docx';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            column(WorkDate; Format(Workdate(), 0, 4))
            { }
            column(Text001Lbl; Text001Lbl)
            { }
            column(Text002Lbl; Text002Lbl)
            { }
            column(Text003Lbl; Text003Lbl)
            { }
            column(Text004Lbl; Text004Lbl)
            { }
            column(Text005Lbl; Text005Lbl)
            { }
            column(Text006Lbl; Text006Lbl)
            { }
            column(Text007Lbl; Text007Lbl)
            { }
            column(Text010Lbl; Text010Lbl)
            { }
            column(vShipmentMethodName; vShipmentMethodName)
            { }
            column(vfechaETDPI; Format((vFechaETDPI), 0, 3)) //Format(CalcDate('+18D', "BBT ETD PI"), 0, 3))
            { }
            column(vfechaETAPlanning; Format("BBT ETD PI", 0, 3))
            { }
            column(vTotalAmount; vTotalAmount)
            { }
            column(vPaymentTerms; vPaymentTerms)
            { }
            column(vPaymentMethod; vPaymentMethod)
            { }
            column(vVendorName; "Buy-from Vendor Name")
            { }
            column(vVendorCounty; "Buy-from County")
            { }
            column(vVendorCountry; UpperCase(vVendorCountry))
            { }
            column(vVendorAddress; "Buy-from Address")
            { }
            column(vCustomerBankAddress2; "Buy-from Address 2")
            { }
            column(vCustomerBankName; vCustomerBankName)
            { }
            column(vCustomerBankIBAN; vCustomerBankIBAN)
            { }
            column(vCustomerSWIFT; vCustomerSWIFT)
            { }
            column(vCustomerBankAddress; vCustomerBankAddress)
            { }
            column(vPortLoading; vPortLoading)
            { }
            ...
            dataitem(Line; "Purchase Line")
            {
                ...
                { }
                column(vTariffCode; vTariffCode)
                { }
                ...
                begin
                    ...
                    FormatDocument.SetPurchaseLine(Line, FormattedQuanitity, FormattedDirectUnitCost, FormattedVATPct, FormattedLineAmount);
                    if rlItem.Get("No.") then vTariffCode := rlItem."Tariff No.";
            ...
            trigger OnAfterGetRecord()
            var
                rlCustomerBankAccount: Record "Vendor Bank Account";
                rlPaymentTerms: Record "Payment Terms";
                rlPaymentMethod: Record "Payment Method";
                rlPurchaseLine: Record "Purchase Line";
                rlPurchaseLine2: Record "Purchase Line";
                rlShipmentMethod: Record "Shipment Method";
                rlCountry: Record "Country/Region";
                rlShipmentMethod2: Record "Shipment Method";
            begin
                TotalAmount := 0;
                TotalSubTotal := 0;
                ...
                if rlCustomerBankAccount.Get("Purchase Header"."Buy-from Vendor No.", "Purchase Header"."Vendor Bank Acc. Code") then begin
                    vCustomerBankName := rlCustomerBankAccount.Name;
                    vCustomerBankIBAN := rlCustomerBankAccount.IBAN;
                    vCustomerSWIFT := rlCustomerBankAccount."SWIFT Code";
                    vCustomerBankAddress := rlCustomerBankAccount.Address + ' ' + rlCustomerBankAccount."Address 2";
                end;
                if rlPaymentTerms.Get("Purchase Header"."Payment Terms Code") then vPaymentTerms := rlPaymentTerms.Description;
                if rlPaymentMethod.Get("Purchase Header"."Payment Method Code") then vPaymentMethod := rlPaymentMethod.Description;
                rlPurchaseLine.Reset();
                rlPurchaseLine.SetRange("Document Type", rlPurchaseLine."Document Type"::Order);
                rlPurchaseLine.SetRange("Document No.", "Purchase Header"."No.");
                if rlPurchaseLine.FindSet() then begin
                    rlPurchaseLine.CalcSums("Amount Including VAT");
                    vTotalAmount := rlPurchaseLine."Amount Including VAT";
                    if rlShipmentMethod.Get("Purchase Header"."Shipment Method Code") then vShipmentMethodName := rlShipmentMethod.Description;
                    if rlCountry.Get("Purchase Header"."Buy-from Country/Region Code") then vVendorCountry := UpperCase(rlCountry.Name);
                    rlPurchaseLine2.Reset();
                    rlPurchaseLine2.SetRange("Document Type", rlPurchaseLine."Document Type"::Order);
                    rlPurchaseLine2.SetRange("Document No.", "Purchase Header"."No.");
                    rlPurchaseLine2.SetFilter("Port - POL", '<>%1', '');
                    if rlPurchaseLine2.FindFirst() then if rlShipmentMethod2.Get(rlPurchaseLine2."Port - POL") then vPortLoading := rlShipmentMethod2.Description;
                    if "Purchase Header"."BBT ETD PI" <> 0D then
                        vFechaETDPI := CalcDate('+18D', "BBT ETD PI")
                    else
                        Clear(vFechaETDPI);
                    if vShowPLVersion then begin
                        Text001Lbl := 'GDANSK (POLAND)';
                        Text002Lbl := 'INCLUDING SPARE PARTS/GW/NW/CBM';
                        Text003Lbl := '5263290009, ADDRESS C/CATALUNYA, 24. P.IND. CA N’OLLER 08130 STA. PERPÈTUA DE MOGODA (BARCELONA)';
                        Text004Lbl := '';
                        Text005Lbl := '';
                        Text006Lbl := '.';
                        Text007Lbl := '';
                        ...
                        Text001Lbl := 'BARCELONA, SPAIN';
                        Text002Lbl := '';
                        Text003Lbl := '';
                        //Text011Lbl := '9.- ORIGINAL NON-RECYCLED PLASTIC AS FULLFILMENT DECLARATION OF WASTE AND CONTAMINATED LAND ACT, ISSUED AND SIGNED BY MANUFACTURER OR BENEFICIARY.';
                        Text011Lbl := '';
                        Text004Lbl := '';
                        //Text004Lbl := '' + vLChar[1] + vLChar[2] + '' + vLChar[1] + vLChar[2] + Text011Lbl;
                        //Text005Lbl := '8';
                        Text005Lbl := '9.- ORIGINAL NON-RECYCLED PLASTIC AS FULLFILMENT DECLARATION OF WASTE AND CONTAMINATED LAND ACT, ISSUED AND SIGNED BY MANUFACTURER OR BENEFICIARY.';
                        Text006Lbl := 'AND THE TOTAL AMOUNT OF PACKAGING CONTAINING NON-RECYCLED PLASTIC USED IN THIS PURCHASING ORDER. THE INVOICE MUST SHOW:';
                        Text007Lbl := '-	IF THE DOCUMENT OF TRANSPORT IS AWB THE CORRECT INCOTERM MUST BE FCA.';
                        Text008Lbl := ' •	RII RAEE NUMBER: 5163';
                        Text009Lbl := ' •	RII PyA NUMBER: 1153';
                        Text010Lbl := Text006Lbl + vLChar[1] + vLChar[2] + Text008Lbl + vLChar[1] + vLChar[2] + Text009Lbl;
                    end;
                end;
            end;
        }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ShowPLVersion; vShowPLVersion)
                    {
                        ApplicationArea = Suite;
                        Caption = 'Show PL Credit Letter', Comment = 'ESP="Mostrar versión PL"';
                    }
    ...
    var
        vShipmentMethodName: Text;
        vPortLoading: Text;
        GLSetup: Record "General Ledger Setup";
        vTotalAmount: Decimal;
        vCustomerBankName: Text;
        Text001Lbl: Text;
        Text002Lbl: Text;
        Text003Lbl: Text;
        Text004Lbl: Text;
        ...
        Text011Lbl: Text;
        vVendorCountry: Text;
        vCustomerBankIBAN: Text;
        vCustomerBankAddress: Text;
        vCustomerBankAddress2: Text;
        vCustomerSWIFT: Text;
        vPaymentTerms: Text;
        vPaymentMethod: Text;
